...
int main(){
	Mat segmented = imread("../Dataset/game1_clip1/masks/frame_last.png", IMREAD_GRAYSCALE);
	...
	const string str = "../Dataset/game1_clip1/bounding_boxes/frame_last_bbox.txt";
	ifstream file(str);
	Table table;
	vector<Ball> bboxes;
	string line;
	...
	compareMetrics(table, segmentedColored, "../Dataset/game1_clip1", LAST);
	waitKey();
	return 0;
}
